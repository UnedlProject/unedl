...
class ClientesController extends Controller
{
    ...
    public function getClientes(){
        ...
        //dd($events);
        return view('ViewClientes',compact('clientes'));
    }
    public function getpuestosClientes(){
        $puesto = Puesto::orderBy('created_at', 'asc')->get();
        $indicador=Indicador::all();
        return view('cliente', ['puesto' => $puesto,'indicador'=>$indicador]);
    }
    public function updateStatus(Request $request){
        $id=$request->input('id');
        $cliente=Cliente::where('id',$id)->first();
        $status = $cliente->status;
        $cliente->status = $status == 'enabled' ? 'disabled' : 'enabled';
        $cliente->save();
        return redirect()->back();
    }
    public function createClientes(FinancierosRequest $request){
        $proceso=new Cliente();
        ...
        if($proceso->save()){
            ...
            return redirect('cliente')->with(['message'=>$message]);
        }
    }
    public function ClientesUpdate(Request $request)
    {
        $id=$request->input('id');
        $informacion = Cliente::where('id',$id)->first();
