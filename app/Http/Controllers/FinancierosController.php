...
class FinancierosController extends Controller
{
    public function index(){
        return view('financiero');
    }
    public function getFinancieros(){
        $financieros = Financiero::orderBy('created_at','asc')->paginate(5);
        //dd($events);
        return view('ViewFinanciero',['financiero' => $financieros]);
    }
    public function getpuestosFinancieros(){
        $puesto = Puesto::orderBy('created_at', 'asc')->get();
        $indicador=Indicador::all();
        return view('financiero', ['puesto' => $puesto,'indicador'=>$indicador]);
    }
    public function updateStatus(Request $request){
        $id=$request->input('id');
        $financiero=Financiero::where('id',$id)->first();
        $status = $financiero->status;
        $financiero->status = $status == 'enabled' ? 'disabled' : 'enabled';
        $financiero->save();
        return redirect()->back();
    }
    public function createFinancieros(FinancierosRequest $request){
        $proceso=new Financiero();
        ...
        if($proceso->save()){
            ...
            return redirect('financiero')->with(['message'=>$message]);
        }
    }
    public function ErroresUpdate(Request $request)
    {
        $id=$request->input('id');
        $informacion = Financiero::where('id',$id)->first();
