...
class ProcesosController extends Controller
{
    ...
    public function getProcesos(){
        $homes = Proceso::orderBy('created_at','asc')->paginate(5);
        //dd($events);
        return view('ViewHome',['home' => $homes]);
    }
    public function getpuestosProcesos(){
        $puesto = Puesto::orderBy('created_at', 'asc')->get();
        $indicador=Indicador::all();
        return view('home', ['puesto' => $puesto,'indicador'=>$indicador]);
    }
    public function updateStatus(Request $request){
        $id=$request->input('id');
        $proceso=Proceso::where('id',$id)->first();
        $status = $proceso->status;
        $proceso->status = $status == 'enabled' ? 'disabled' : 'enabled';
        $proceso->save();
        return redirect()->back();
    }
    public function createProcesos(FinancierosRequest $request){
        $proceso=new Proceso();
    ...
    {
        $id=$request->input('id');
        $informacion = Proceso::where('id',$id)->first();
